...
const reloadArticles = (keyword) => {
    ...
    const articlesToShow = data.articles.filter(article => article.tags.includes(keyword));
...
const cleanedKeyword = (keyword) => {
    const cleanedKeyword = keyword.toLowerCase().replace(/[^\w]/gi, '');
...
const showKeywordsList = (value, allKeywords, keywordsCategories) => {
    // Starting at 3 letters inserted in the form, we do something
    const keyword = allKeywords.filter(tag => cleanedKeyword(tag).startsWith(value))[0];
    if (value.length >= 3 && keyword !== undefined) {
        const keyWordUl = document.querySelector(".inputKeywordsHandle ul");
        resetKeywordsUl();
        const categoryOfKeyword = keywordsCategories //à suivre
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    inputElement.addEventListener('input', (event) => {
        ...
        showKeywordsList(cleanedKeyword(value), allKeywords, keywordsCategories);
    });
